...
const Terminal = upstream.Terminal;
const ansi = upstream.ansi;
const keyboard = upstream.input.keyboard;
const Context = upstream.Context;
const Options = upstream.Options;
const message = upstream.msg;
const command = upstream.command;
const Logger = upstream.Logger;
const unicode = upstream.unicode;
    ...
    return struct {
        ...
        pub fn initWithOptions(
            ...
            const self = Self{
                ...
                .pending_image = null,
                .logger = null,
                .input_parser = InputParser.init(allocator),
                .filter = null,
            };
        ...
        pub fn tick(self: *Self) !void {
            ...
            if (bytes_read > 0) {
                const events = try self.input_parser.parseChunk(self.context.allocator, input_buf[0..bytes_read]);
                for (events) |event| {
                    try self.processInputEvent(event);
                }
            }
        ...
        fn processInputEvent(self: *Self, event: keyboard.ParseResult) !void {
        ...
        fn processCommand(self: *Self, cmd: UserCmd) !void {
            switch (cmd) {
                ...
                .delete_image => |del| {
                    if (self.terminal) |*term| {
                        const target: upstream.terminal.KittyDeleteTarget = switch (del) {
                            .by_id => |id| .{ .by_id = id },
        ...
        fn flushPendingImage(self: *Self) !void {
            const TerminalMod = upstream.terminal;
